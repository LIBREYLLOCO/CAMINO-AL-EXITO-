...
const CardModal: React.FC<CardModalProps> = ({ card, category, onResolve }) => {
    ...
    return (
        <div className="fixed inset-0 flex items-center justify-center p-6 bg-black/95 backdrop-blur-md z-[100] modal-active">
            <div className={`w-full max-w-sm rounded-[2.5rem] p-6 flex flex-col text-center shadow-2xl border-4 border-white/10 animate__animated animate__zoomIn text-slate-900 max-h-[85vh] ${card.c || 'bg-white'}`}>
                ...
                {showActionButtons ? (
                     <div className="grid grid-cols-2 gap-3 w-full">
                        <button onClick={() => onResolve(true)} className="py-4 bg-green-500 text-white rounded-xl font-black uppercase shadow-lg text-xs hover:bg-green-600">✅ ¡LOGRADO!</button>
                        <button onClick={() => onResolve(false)} className="py-4 bg-red-500 text-white rounded-xl font-black uppercase shadow-lg text-xs hover:bg-red-600">❌ FALLÉ</button>
                    </div>
                ) : (
                    <button onClick={() => onResolve(true)} className="w-full py-4 bg-slate-900 text-white rounded-xl font-black uppercase tracking-widest shadow-lg">Entendido</button>
                )}
            </div>
        </div>
    );
